...
const MONO_REMOTE = 'sps-cli';
// sps-cli is a subdirectory of the WeWaMo monorepo; __dirname is sps-cli/dist/sync-code
const MONO_ROOT = path.resolve(__dirname, '..', '..', '..').replace(/\\/g, '/');
// The sps-cli source subdirectory (relative to monorepo root)
const SPS_CLI_PREFIX = path.relative(MONO_ROOT, path.resolve(__dirname, '..', '..')).replace(/\\/g, '/');

function gitSubtreePush(monoRoot: string, subtreePrefix: string): void {
  const branch = `dev-push-${Date.now()}`;
  exec(`git -C "${monoRoot}" subtree split --prefix="${subtreePrefix}" -b "${branch}"`, {});
  exec(`git -C "${monoRoot}" push --force sps-cli ${branch}:main`, {});
  exec(`git -C "${monoRoot}" branch -D "${branch}"`, {});
}

export async function devPush(localPath: string, message?: string): Promise<void> {
  const remoteUrl = `https://github.com/${SPS_CLI_REPO}.git`;

  // subtreePrefix is always the sps-cli subdirectory (derived from __dirname, not cwd)
  const subtreePrefix = SPS_CLI_PREFIX;

  // 2. Add or update the sps-cli remote in the monorepo
  const git = (args: string) => exec(`git -C "${MONO_ROOT}" ${args}`, {});
  try {
    git(`remote add ${MONO_REMOTE} "${remoteUrl}"`);
  } catch {
    git(`remote set-url ${MONO_REMOTE} "${remoteUrl}"`);
  }

  // 3. Check for changes in sps-cli subdirectory
  const files = git(`diff --name-only -- "${subtreePrefix}"`);
  if (!files.trim()) {
    throw new Error('No changes to push');
  }
  ...
  console.log(chalk.cyan('─'.repeat(60)));
  console.log(chalk.green(`Files to push: ${changedFiles.length}`));
  console.log(chalk.cyan('─'.repeat(60)));
  changedFiles.forEach(f => console.log(`  ${chalk.yellow(f)}`));
  console.log(chalk.cyan('─'.repeat(60)));
  ...
  // 6. Commit in monorepo if there are uncommitted changes
  const status = git(`status --porcelain -- "${subtreePrefix}"`);
  if (status.trim()) {
  ...
  // 7. Push via git subtree
  gitSubtreePush(MONO_ROOT, subtreePrefix);
  console.log(chalk.green(`✓ Pushed sps-cli to ${SPS_CLI_REPO}/main`));
}
